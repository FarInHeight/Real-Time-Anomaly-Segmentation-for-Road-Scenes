...
class cityscapes(Dataset):
    def __init__(self, root, input_transform=None, target_transform=None, subset='val/'):
        ...
        self.filenames = [
            dp + '/' + f for dp, _, fn in os.walk(os.path.expanduser(self.images_root)) for f in fn if is_image(f)
        ]
        # sort the image filenames
        self.filenames.sort()
        ...
        self.filenamesGt = [
            dp + '/' + f for dp, dn, fn in os.walk(os.path.expanduser(self.labels_root)) for f in fn if is_label(f)
        ]
